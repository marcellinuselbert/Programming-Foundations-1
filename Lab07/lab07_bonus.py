...
while True:
    ...
    elif "TUJUAN_KELAS_TERMURAH" in user_choice:
        user_choice = user_choice.replace("TUJUAN_KELAS_TERMURAH", "")
        ...
        if len(user_choice_splitted) !=2:
            ...
            for schedule in list_of_schedules:
                if schedule["tujuan_akhir"] == user_choice_splitted[
                    ...
                    match += 1
                    # for every filtered tickey by kelas and tujuan, append the schedule harga tiket
                    list_of_cost.append(schedule["harga_tiket"])
            # get the min value of harga ticket
            min_cost = min(list_of_cost)
            for schedule in list_of_schedules:
                # print all the lowest harga ticket
                if schedule["harga_tiket"] == min_cost:
    ...
    # split by space into list then access the index, index 0 for tujuan, index 1 for kelas
        ...
        if match == 0 and len(user_choice_splitted) ==2:
            print("Tidak ada jadwal kereta yang sesuai ")
    elif "TUJUAN_JAM_TERMURAH" in user_choice:
        user_choice = user_choice.replace("TUJUAN_JAM_TERMURAH", "")
        user_choice_splitted = user_choice.split()
        match = 0
        if len(user_choice_splitted) !=2:
            print("Perintah yang dimasukkan tidak valid.")
            ...
            for schedule in list_of_schedules:
                if schedule["tujuan_akhir"] == user_choice_splitted[
                    ...
                    match += 1
                    # for every filtered tickey by jam and tujuan, append the schedule harga tiket
                    list_of_cost.append(schedule["harga_tiket"])
            # get the min value
            min_cost = min(list_of_cost)
            for schedule in list_of_schedules:
                # print all the min value harga ticket only
                if schedule["harga_tiket"] == min_cost:
                    print("KA {} berangkat pukul {} dengan harga tiket {}".
                          format(schedule["nomor_ka"],
                                 schedule["jam_keberangkatan"],
